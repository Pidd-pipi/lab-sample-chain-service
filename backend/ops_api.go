...
func newOpsAPIHandler(service *OpsService, gate opsGate) *opsAPIHandler {
	return &opsAPIHandler{service: service, gate: gate}
}
...
func (h *opsAPIHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost && !h.gate.Allow(r) {
		opsJSON(w, http.StatusForbidden, map[string]string{"error": "operator required"})
		return
	}
	switch {
...
func (h *opsAPIHandler) create(w http.ResponseWriter, r *http.Request) {
	...
	if record.ID == "" {
		record.ID = fmt.Sprintf("ops-%d", opsCreateSequence())
	}
	record = enrichOpsLabels(record)
